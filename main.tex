...
\\
\begin{equation}
  G_{B} = g_{0}\frac{(\frac{\Gamma_{0}}{2})^{2}}{(\Omega - \Omega_{B})^{2} + (\frac{\Gamma_{0}}{2})^{2}}
\end{equation}
\\
with the on-resonance gain factor of the material is given by
\\
\begin{equation}
  g_{0} = [\frac{\epsilon_{0}^{2}\omega^{2}q^{4}\gamma_{e}^{4}}{4n^{2}c^{2}\rho_{0}^{2}}]
\end{equation}
$q$ is the wavevector of the driven acoustic field, $\gamma_{e}$ is the electrostrictive constant, $\rho_{0}$ is the mean density of the material, $\Omega_{B}$ is the resonant Brillouin frequency of the material, $\Gamma_{B}$ is the Brillouin linewidth, or dissipation rate, of the material, and $\Delta k$ is the wavevector mismatch between the optical fields.
