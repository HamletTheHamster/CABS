...
\caption{
Design diagram for a coherently stimulated phonon spectrometer. A tunable CW laser supplies approximately 1.55$\mu$m light which is sent through an isolator (Iso) to a splitter, which routes 10\% of the light through a 27 dbm EDFA followed by a VOA. This Pump light ($\omega_P$) is polarization controlled to reflect at a PBS, recouple to fiber via a fiber port (FP), and continue on to a sample via either direct coupling for fiber samples or by passing through a pair of FPs and lenses (L) straddling a free-space sample. Pump light transmits through a sample and continues through a corrective polarization controller which accounts for twists and bends of the fiber in the sample stage before reflecting at a second PBS where it is circulated to a power meter at port (c) for monitoring. We synthesize a Stokes wave from a 90\% split of the original Pump laser, using an intensity modulator (IM) and fiber bragg grating (FBG) to generate and select a lower frequency Stokes sideband shifted from the Pump by $\Omega$, a parameter that is swept through a range via a signal generator to capture $\Omega_B$. A 99/1 splitter provides a tap at port (a) for optimizing synthesis of Stokes light, which is amplified by a 1 W EDFA and power-controlled by a VOA. This Stokes wave ($\omega_S$) counter-propagates along the same path as the Pump until being circulated to a power meter at port (b) for monitoring. A second tunable CW laser supplies light detuned from the Pump to serve as a Probe wave ($\omega_{Pr}$), which is amplified by another 1 W EDFA, variably-attenuated, and polarization-controlled to pass through the first PBS, whereby it is incident on a sample via a configuration tailored to the sample. Backscattered Signal light ($\omega_{sig}$) traces back through the first PBS while unscattered Probe light transmits through a sample to be collected by a power meter at port (d) for monitoring. A circulator separates the Signal from the path of the Probe after which a FBG filters out any unwanted noise or Stokes light from the Signal. The Signal is optically heterodyned with an EDFA-amplified and AOM-shifted LO synthesized from the Probe laser before shining on a photodiode for detection. The RF signal is further down-converted to a fixed frequency for collection by a lock-in amplifier via an RF mixer which takes an AC LO signal from the signal generator sweeping synchronously with the Stokes-synthesis AC signal.
}
