...
Fig. \ref{fig:10fWSensitivity} shows the Brillouin signal for a sensitivity of $P_{Signal} = [10]\,fW$. We find the signal-to-noise ratio (SNR) of this measurement to be $[3]$, determined by taking the ratio of the peak spectral density at the center frequency ($9.18 GHz$) and the standard error in spectral density completely off resonance ($9.3 GHz$). Assuming a Gaussian noise distrubution profile, an SNR of $[3]$ is equivalent to a $[3]\sigma$ confidence interval, or 99.7\% confidence in the statistical significance of this measurement.
