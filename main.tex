...
For the measurements of 100 $\mu$m $CS_{2}$ (Fig. \ref{fig:100umCS2}) and 1 cm UHNA3 at low power (Fig. \ref{fig:5fWSensitivity}), the amplitude of the resonant Brillouin peak is only ~1x to 2x that of the non‐resonant continuum, giving a strong chance for Fano interference. Indeed it was observed for 100 $\mu$m of $CS_2$ that with a small change in the probe laser's wavelength, and thus a slightly different pump-probe detuning, an asymmetric lineshape was observed. A particularly striking example of this Fano asymmetry is found in our measurements of 1 mm of $CS_2$, shown in Appendix \ref{Appendix:Fano}, in which we performed a similar phase-matching experiment as was done for 1 cm UHNA3 (see preceding subsection). There, we see line shapes with pronounced asymmetry featuring characteristic morphologies for various values of $q$. These pronounced distortions in spectral line shape for small signal measurements underscore the role of Fano interference in small resonant amplitudes relative to the background.

In the phase-matching bandwidth experiment performed on 1 cm UHNA3 (Fig. \ref{fig:Phase-Match} and described in previous subsection), several factors contribute to a slight deviation from the simple theory of a pure $sinc^2$ function, including noise floor, alignment drifts, fiber dispersion, among possible others. We largely see the $sinc^2$ dependence, however at larger pump‐probe detunings (above 15 GHz), the measured peaks become quite weak, and a slight spectral asymmetry emerges (see Appendix \ref{Appendix:Fano}). This is consistent with a Fano‐type distortion, wherein the signal amplitude is comparable to the underlying continuum, leading to an interference phenomenon that slightly skews the measured peak. Because of the Fano line shape, the raw peak amplitude from the spectral trace is not exactly at the Lorentzian center. A naive Lorentzian fit would thus not accurately capture an accurate peak amplitude of these larger pump-probe detuned sets. We corrected for this by fitting a standard Fano profile, which accurately captures the peak amplitudes, however the Fano-resonant effects serve to amplify or diminish the underlying Brillouin response depending on the sign of the Fano asymmetry parameter $q$ (e.g. whether the Brillouin response slightly lags or leads the broad background continuum in phase, respectively).
