...
We first set out to test the sensitivity of the instrument, which could then be applied to any specific future measurement. In terms of Eq. \ref{Eq:Theoretical Framework:Scattered Power}, the sensitivity of the instrument is the minimum detectable scattered power, $P_{Signal}$. By targeting a specific length, $L$ of a sample of known effective Brillouin gain, $G_{B}$, the scattered power can be determined for given pump, Stokes, and probe optical powers as well as pump-probe detuning, $\Delta\lambda$. The sensitivity floor is obtained when the minimum multiplicative combination of optical powers that produces a distinguishable signal is found.
