...
For a free-space bulk example we target carbon disulfide liquid for its exceptionally high Brillouin gain factor of 1.5 m/GW.\cite{boyd2020nonlinear} Figure \ref{fig:100umCS2} reveals the Brillouin signal of carbon disulfide liquid contained in a 100 $\mu$m path length cell. Measurement of travelling-wave phonons in such a small path length has never before been reported in the literature. A scattered power comparison reveals that a measurement of this kind using balanced detection, the current leading SBS technique, would only be achievable at cryogenic temperatures. 
